...
- Anything touching playback is checked by hand with a player running — look at `playing` first and
  ...
  --progressive`, `release` 3 s later: about +180 s and the line ends with `×1.6`), and `seek` past
  the end. Do `seek` past the end last and on something disposable: it ends the item, and a page
