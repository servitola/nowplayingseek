...
Every command is one line in, and the exit code is the truth: `0` it happened, `1` nothing is
playing, `2` the player ignored it, `64` bad arguments, `78` bad config — the same `0` behind the
dialect names, `nowplaying-cli` and `media-control`.
...
The three flags work on every command that reads or moves the player: `seek 12:30 --json`,
`forward --minify`, `position --json`, `doctor --json`, `config --json`. `--json` is indented in a
pipe as in a terminal; only a terminal gets colour. The `human` object at the end repeats
`position`, `duration`, `timestamp` and `app` in words — parse with `jq`, or take `--minify` and
the first match. The order of flags does not matter; `--raw` with `--json` is `--raw`.
...
Only the [elected app](../README.md#limits) can be addressed. When `status` shows the wrong one,
do not retry — tell the person to press play there instead.
